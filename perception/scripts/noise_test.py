...
class Noise:
    def __init__(self):
        ...
        # Subscribers
        rospy.Subscriber("/camera/depth/image_raw", Image, self.handle_image)
        self.bridge = CvBridge()
        self.img_pub = rospy.Publisher("/depth/image/noise", Image, queue_size=1)
        self.dot_pattern_ = cv2.imread("../data/kinect-pattern_3x3.png", 0)
    ...
    def handle_image(self, msg):
        t = time.time()
        depth_uint16 = self.bridge.imgmsg_to_cv2(msg, desired_encoding='passthrough')
        ...
        out_disp = self.filterDisp(depth_f, self.dot_pattern_, invalid_disp_)
        ...
        noisy_depth = (35130 / np.round((35130 / np.round(depth * scale_factor)) + np.random.normal(size=(h, w)) * (
                    1.0 / 6.0) + 0.5)) / scale_factor
        ...
        # print(depth_uint16.shape)
        # depth2save = np.hstack((depth_uint16, noisy_depth))
        # print(depth2save.shape)
        # cv2.imwrite('depth_noised_{}.png'.format(count), depth2save)
        # after_bridge = self.bridge.cv2_to_imgmsg(noisy_depth)
        # msg.data = after_bridge.data
        # msg.height = noisy_depth.shape[0]
        # msg.width = noisy_depth.shape[1]
        # self.img_pub.publish(after_bridge)
        print("One image is handled in ", time.time()-t)
